...
% this version uses complete linkage distance for inter-cluster distance
% (distance between two FURTHEST data points from different clusters) and
...
function Dunn = eeg_Dunn2(IndSamples, ClustLabels)
    ...
    for i=1:numClusts
        for j=1:numClusts
            clust1 = find(ClustLabels==i);
            clust2 = find(ClustLabels==j);
            dists = distM(clust1, clust2);
            interClustDists = [interClustDists; max(dists(:))];
        end
    end
