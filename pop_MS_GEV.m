function com = pop_MS_GEV(AllEEG,MeanSet, IgnorePolarity)
    com = '';
    ...
    GEVExplorer(eegout, AllEEG(ChildIndex));    
    txt = sprintf('%i ',SelectedMean);
    txt(end) = [];
    
        
    com = sprintf('com = pop_MS_GEV(%s, [%s], %i);', inputname(1),txt,IgnorePolarity); 
...
function ChildIndex = FindTheWholeFamily(TheMeanEEG,AllEEGs)
    ...
    for i = 1:numel(TheMeanEEG.msinfo.children)
        ...
        if isempty(idx)
            errordlg2(sprintf('Dataset %s not found',TheMeanEEG.msinfo.children{i}),'GEV explorer');
        end

        if numel(idx) > 1
            errordlg2(sprintf('Dataset %s repeatedly found',TheMeanEEG.msinfo.children{i}),'GEV explorer');
        end
