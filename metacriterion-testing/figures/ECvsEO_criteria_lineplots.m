...
figure
testCriteriaValues = criteria(matches({criteria.name}, 'KL')).ECvalues';
for i=1:nSubjects/2
    plot(ClusterNumbers, testCriteriaValues(:,i));
    hold on
end

% make line plots
% figureName = sprintf('EC Criteria Lineplots - %s', dataName);
...
%     nexttile;
%     plot(ClusterNumbers, criteria(i).ECvalues');
%     title(criteria(i).fullname, 'fontsize', fontSize);
% end
% filename = fullfile(scriptPath, 'EC vs EO criteria line plots', sprintf('EC Criteria Lineplots_%s.fig', dataName));
% saveas(gcf, filename);
% 
% figureName = sprintf('EO Criteria Lineplots - %s', dataName);
% figure('Name', figureName);
% tiledlayout(3,3);
% fontSize = 20;
% 
% for i=1:numel(criteria)
%     nexttile;
%     plot(ClusterNumbers, criteria(i).EOvalues');
%     title(criteria(i).fullname, 'fontsize', fontSize);
% end
% filename = fullfile(scriptPath, 'EC vs EO criteria line plots', sprintf('EO Criteria Lineplots_%s.fig', dataName));
% saveas(gcf, filename);