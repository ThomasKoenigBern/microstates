...
function vers = eegplugin_Microstates (fig, try_strings, catch_strings)
    ...
    comQuantMSTransitionPrb= [try_strings.no_check '[           LASTCOM] = pop_QuantMSTemplates(ALLEEG,CURRENTSET,0, [], [], 1);'   catch_strings.new_and_hist];
...
%    uimenu( toolssubmenu, 'Label', 'Crossvalidate optimal number of classes'              , 'CallBack', comBootStrapMSNumber , 'position', 4, 'userdata', 'study:on');
    ...
    uimenu( toolssubmenu, 'Label', 'Display microstate transition probabilities (own template maps)'  , 'CallBack', comQuantMSTransitionPrb,   'position', 15, 'Separator','on');
